...
const ChatList = ({ search }) => {
  const { users, unseenMessages, setSelectedUser, setSelectedGroup, setUnseenMessages, setShowInfoDrawer, selectedUser } = useContext(ChatContext);
  ...
  return (
    <>
      {filteredUsers.map((user) => {
        ...
        const directCount = unseenMessages ? unseenMessages[userIdString] || 0 : 0;
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col leading-5">
                ...
                <span className={`text-xs ${onlineUsers?.includes(user._id) ? "text-green-400" : "text-neutral-400"}`}>
                  {iBlockedThem ? "Blocked" : theyBlockedMe ? "Unavailable" : onlineUsers?.includes(user._id) ? "Online" : "Offline"}
                </span>
              </div>
            </div>
            
            {iBlockedThem && (
              <span className="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded border border-red-500/30">You Blocked</span>
            )}
            {theyBlockedMe && !iBlockedThem && (
              <span className="text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded border border-amber-500/30">Blocked You</span>
            )}
