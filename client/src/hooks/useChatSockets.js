...
export const ChatProvider = ({ children }) => {
    ...
    (eventName, payload, timeout = 30000) =>
      new Promise((resolve, reject) => {
        ...
        socket.timeout(timeout).emit(eventName, payload, (error, response) => {
          ...
          if (!response?.success) return reject(new Error(response?.message || "Socket action failed"));
          resolve(response);
        });
      }),
    [socket]
  ...
  const markAsSeen = useCallback(
    async (chatId, isGroupChat = false) => {
      ...
      try {
        await emitSocketAction("markMessagesSeen", { chatId, isGroup: isGroupChat });
      } catch (err) {
  ...
  const sendMessage = async (formData, customTargetId = null, customIsGroup = null) => {
    try {
      const isGroupChat = customIsGroup !== null ? customIsGroup : !!selectedGroup;
      const targetId = customTargetId || (isGroupChat ? selectedGroup?._id : selectedUser?._id);
      ...
      if (fileAsset) {
        ...
        if (uploadData.mimetype.startsWith("image/")) autoDetectedType = "image";
        else if (uploadData.mimetype.startsWith("audio/")) autoDetectedType = "audio";
        else if (uploadData.mimetype.startsWith("video/")) autoDetectedType = "video";
      }
      ...
      console.error(error);
      toast.error(error.response?.data?.message || "Failed to dispatch message");
    }
  };
  ...
  const handleMessageReaction = async (messageId, emoji) => {
    ...
    setMessages((prev) =>
      prev.map((msg) => {
        ...
        const existingIndex = reactions.findIndex(
          (r) => (r.userId?._id || r.userId)?.toString() === authUser._id.toString()
        );

        let updatedReactions = [...reactions];
        if (existingIndex > -1) {
          if (updatedReactions[existingIndex].emoji === emoji) updatedReactions.splice(existingIndex, 1);
          else updatedReactions[existingIndex] = { ...updatedReactions[existingIndex], emoji };
        } else {
    ...
    try {
      ...
      console.error("Failed handling syncing reaction setup configurations:", error);
    }
  };
  ...
  const addGroupMember = async (groupId, userId) => {
    try {
      ...
      setSelectedGroup((prev) => ({ ...prev, members: [...prev.members, res.newMember] }));
  ...
  const handleAdminAction = async (groupId, applicantId, action) => {
    try {
      const data = await groupService.resolveRequest(groupId, applicantId, action);
  ...
  const forwardMessage = async (messageToForward, targetId, isGroupTarget) => {
    try {
      ...
      if (messageToForward.image) formData.append("image", messageToForward.image);
      if (messageToForward.audio) formData.append("audio", messageToForward.audio);
      if (messageToForward.video) formData.append("video", messageToForward.video);
  ...
  const sendTypingStatus = (isTyping, forcedChatId = null, forcedIsGroup = null) => {
    if (!socket) return;
    const resolveIsGroup = forcedIsGroup !== null ? forcedIsGroup : !!selectedGroup;
    const targetChatId = forcedChatId || (resolveIsGroup ? selectedGroup?._id : selectedUser?._id);
